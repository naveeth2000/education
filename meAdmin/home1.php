		...
		<?php $teacher_dis_admin = $naveeth->teacher_info_display_admin();
		$t_sn = 1;
	...
	if(isset($_POST['update_admin_info'])){
		...
		$checkmatch = $naveeth->meadmin_check($current_username,$current_password);
		if($checkmatch){
			$success = $naveeth->meadmin_updatepassword($new_password,$new_username);
			if($success){
	...
	?>
		...
		<label><?php $count_student_count=  $naveeth->student_count(); 
		...
		<div class="stats-right">
		<label><?php 
		$count_teacher_size=  $naveeth->teacher_info_display_admin(); 
